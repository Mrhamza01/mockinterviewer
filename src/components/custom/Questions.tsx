    ...
    <div className="p-5 border rounded-lg my-10">
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
        {data && data.map((item:any, index:number) => (
          <h2
            key={index}
            className={`p-2 rounded-full text-xs md:text-sm text-center cursor-pointer ${activeQuestionIndex === index ? 'bg-blue-700 text-white' : 'bg-secondary'}`}
      ...
      <Volume2 className="cursor-pointer" onClick={() => TextToSpeech(data[activeQuestionIndex]?.question)} />
